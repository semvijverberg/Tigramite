...
RV_final = np.ma.mean(RV_masked, axis = (0,1))
#=====================================================================================
...
outd = dict()
class act:
    def __init__(self, Corr_actor, lat_grid_actor, lon_grid_actor):
        self.Corr_actor = Corr_actor
        self.lat_grid_actor = lat_grid_actor
        self.lon_grid_actor = lon_grid_actor
        
var_names = exp['vars'][0]
for var in var_names:
    ...
    Corr_actor, lat_grid_actor, lon_grid_actor = rgcpd.calc_corr_coeffs_new(ncdf, array, box, RV_index, time_range_all, exp['lag_min'], exp['lag_max'],
                                                                  time_cycle, RV_indices, exp['alpha_fdr'], FDR_control=exp['FDR_control'])
    fig_corr_act1 = rgcpd.plot_corr_coeffs(Corr_actor, m, exp['lag_min'], lat_grid_actor, lon_grid_actor,\
                                            var, Corr_mask=False)
    fig_filename = '{}_vs_{}_lag_{}to{}'.format(var_names[0], var, exp['lag_min'], exp['lag_max']) + params_combination
    plt.savefig(os.path.join(fig_path, fig_filename))
    outd[var] = act(Corr_actor, lat_grid_actor, lon_grid_actor)
    #%%
